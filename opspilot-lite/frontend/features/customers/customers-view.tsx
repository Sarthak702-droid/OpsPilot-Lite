...
const columns: Column<Customer>[] = [{ label: "Customer", render: x => <strong>{x.business_name || x.name}</strong> }, { label: "Customer ID", render: x => <code style={{fontSize:11}}>{x.id}</code> }, { label: "Contact", render: x => x.email || "—" }, { label: "Lifetime revenue", render: x => money(x.lifetime_revenue) }, { label: "Outstanding", render: x => money(x.outstanding) }, { label: "Overdue invoices", render: x => x.overdue_invoice_count }, { label: "Average payment delay", render: x => x.average_payment_delay_days == null ? "—" : `${x.average_payment_delay_days.toFixed(1)} days` }, { label: "Last payment", render: x => x.last_payment_date ? date(x.last_payment_date) : "—" }, { label: "Last purchase", render: x => x.last_purchase ? date(x.last_purchase) : "—" }];
